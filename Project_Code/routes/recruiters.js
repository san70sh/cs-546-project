...
router.post("/login", async (req, res) => {
  ...
  if (email == "" || email == undefined)
    return res.status(400).render("pages/recruiterlogin", {
  ...
  try {
    ...
    return res.render("pages/recruiterlogin", {
      message: e.message,
      mainerr: true,
    });
  }
...
router.get("/accept/:jobId/:appId", async (req, res) => {
  try {
    ...
    return res
      .status(e.status)
      .render("partials/loginform", { message: e.message, err: true });
  }
});
...
router.post("/signup", async (req, res) => {
  ...
  let re = /[A-Z0-9._-]+@[A-Z0-9.-]+\.[A-Z]{2,}/im;
  if (email == "" || email == undefined)
    return res.status(400).render("pages/recruiterSignup", {
      message: "Please enter your email.",
      emailerr: true,
    });
  if (email.length < 6)
  ...
  try {
    ...
    return res
      .status(e.status)
      .render("pages/recruiterSignup", { message: e.message, mainerr: true });
  }
  // }
});
